  ...
  public partial class print_pages : System.Web.UI.Page
  {
    protected void Page_Load(object sender, EventArgs e)
    {
      if (!IsPostBack)
      {
        ...
        r.name AS r_name, d.name AS d_name,
        ...
        ON t.id = tr.tool_id
        INNER JOIN dbo.exam_location el
        ON el.id = tr.exam_location_id
        INNER JOIN dbo.iso i
        ON i.id = tr.iso_id
    ...
    private void RenderData(DataTable dt)
    {
      ...
      Label4.Text = $"แผนก : {dt.Rows[0]["d_name"]} | ฝ่าย : {dt.Rows[0]["c_name"]} | หมายเลขโทรศัพท์ : {dt.Rows[0]["tel"]}";
      Label5.Text = $"{dt.Rows[0]["objective"]}, {Status2Str(dt.Rows[0]["status"].ToString())}, {dt.Rows[0]["iso_name"]}";
      Label6.Text = $"ชื่อเครื่องมือวัด : {dt.Rows[0]["t_name"]} | รหัส : (หน่วยงานสอบเทียบกำหนดให้) {dt.Rows[0]["code"]}";
      Label7.Text = $"ผู้ผลิต : {dt.Rows[0]["pc_name"]} | รุ่น : {dt.Rows[0]["model"]}";
      Label8.Text = $"หมายเลขเครื่อง : {dt.Rows[0]["t_code"]} | ช่วงใช้งาน : {dt.Rows[0]["rang_error"]}";
      Label9.Text = $"สถานที่ใช้งาน : {dt.Rows[0]["location"]} | ค่าผิดพลาดที่ยอมรับได้ : {dt.Rows[0]["accept_error"]}";
