...
{
  public partial class notification_email : System.Web.UI.Page
  {
    ...
    private void CallData(int status)
    {
      ...
      string sql = $@"
          ...
          enf.detail, FORMAT(enf.approved_date , 'dd/MM/yyyy', 'en-us') AS approved_date
          FROM dbo.email_notification enf
      ...
      for (int i = 0; i < dt.Rows.Count; i++)
      {
        string EditBtn = $@"
          <button id=""Approve"" type=""button"" class=""btn btn-sm btn-success""
            onclick=""Approved('notification_email.aspx/Approved',{dt.Rows[i]["id"]})"">
    ...
    {
      bool cb = Model.Database.UpdateByID("dbo.email_notification", "status=1", id);
      return cb;
    }
  }
}